	...
	public class Kelpfish : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			npc.catchItem = (short)mod.ItemType("KelpfishItem");
				banner = npc.type;
			bannerItem = mod.ItemType("KelpfishBanner");
			npc.noGravity = true;
		...
		if(!txt)
		{
			for (int i = 0; i < 4; ++i)
			{
             ...
             dir.Normalize();
             dir *= 1;
             int newNPC = NPC.NewNPC((int)npc.Center.X + (Main.rand.Next(-30, 30)), (int)npc.Center.Y + (Main.rand.Next(-30, 30)), mod.NPCType("Kelpfish1"), npc.whoAmI);
